...
{
//    register_nav_menu('headerMenuLocation', 'Header Menu Location'); // 在主题选项加入menus按钮
//    register_nav_menu('footerLocationOne', 'Footer Location One'); // 在主题选项加入menus按钮
//    register_nav_menu('footerLocationTwo', 'Footer Location Two'); // 在主题选项加入menus按钮
    add_theme_support('title-tag');
    add_theme_support('post-thumbnails');
    add_image_size('professorLandscape', 400, 260, true); // 自动剪切生成为自定义尺寸的图片
...
function university_adjust_queries($query)
{
    if (!is_admin() and is_post_type_archive('campus') and is_main_query()) {
        $query->set('posts_per_page', -1); // 查询所有数据，不分页
    }
